...
public class MFPlayerListener implements Listener {
	...
	public void onPlayerInteract (PlayerInteractEvent event) {
		...
		if (event.getAction().equals(Action.RIGHT_CLICK_BLOCK)) {
			...
			if ((p.getItemInHand().getType() == Material.SNOW_BALL)&&plg.snowball&&p.hasPermission("monsterfix.snowball")) {
				Block tb = p.getTargetBlock((Set<Material>) null, 150);
	...
	public void onPlayerCommandPreprocess (PlayerCommandPreprocessEvent event){
		Player p = event.getPlayer();
		if (plg.cfgB("lcasecmd")){
			String cmd = event.getMessage();
			String [] cln = cmd.split(" ");
			if (cln.length>=1) cmd= cln[0];
			if (!cmd.equals(cmd.toLowerCase())) {
				event.setCancelled(true);
				event.setMessage("Command disabled");
				if (plg.cfgB("lcasecmdmsg")) u.printMSG(p, "msg_lowercasecmd",'c');
				return;
			}
		}
		
		if (plg.cfgB("singlespace")) event.setMessage(event.getMessage().trim().replaceAll("\u0020{2,}", "\u0020"));
	...
	public void onEntityDamageMob (EntityDamageEvent event) {
		if (plg.fixmobfall){
			...
			if (u.isWordInList(e.getType().name(), plg.mfexcept)) return;
			if (event.getCause() == DamageCause.FALL) BukkitCompatibilityFix.setEventDamage(event, 0);
	...
	public void onFallDamage (EntityDamageEvent event) {
		...
		if ((event.getEntity() instanceof Player)&&
			...
			BukkitCompatibilityFix.setEventDamage(event, 0);
	}
	...
	/*
	 *  TODO
	 *  Пермишены! Добавить пермишены на каждый тип прожектила!
	 * 
	 */
		...
		if (evdm.getDamager() instanceof Projectile) {
			...
			if (eye.distance(prj.getLocation())<=1.5D){
				...
				if (prj instanceof Arrow){
					...
					msg = "msg_eggheadshot";
					prjtype=2;
				} else if (prj instanceof FishHook){
				...
				if (BukkitCompatibilityFix.getShooter(prj) instanceof Player){
					shooter = (Player) BukkitCompatibilityFix.getShooter(prj);
					chance = plg.hschance;
					...
					if ((prjtype == 0)&&(!shooter.hasPermission("monsterfix.headshot-arrow"))) return;
					if ((prjtype == 1)&&(!shooter.hasPermission("monsterfix.headshot-snowball"))) return;
					if ((prjtype == 2)&&(!shooter.hasPermission("monsterfix.headshot-egg"))) return;
					if ((prjtype == 3)&&(!shooter.hasPermission("monsterfix.headshot-fishrod"))) return;
					
					
					//if (!shooter.hasPermission("monsterfix.headshot.arrow")) return;

				} else {
					chance = plg.hsmobchance;
					LivingEntity mob_shooter = BukkitCompatibilityFix.getShooter(prj);
					if (mob_shooter == null) shname = u.getMSGnc("someone");
					else shname = mob_shooter.getType().name();
				...
				if (plg.random.nextInt(100)<chance){
					BukkitCompatibilityFix.setEventDamage(event, damage);

 ...
 * 
 * monsterfix.headshot-arrow
   monsterfix.headshot-snowball
   monsterfix.headshot-egg
   monsterfix.headshot-fishrod
 * 
 */
 ...
 *     monsterfix.headshot-arrow
	   ...
	   monsterfix.headshot-egg
	   monsterfix.headshot-fishrod
	 + Удалена перекодировка текста.

	 0.4.1
	 + конфигурация сохраняется в разные файлы
	 + в конфигурации добавлены коментарии
	 + изменена процедура сохранения - теперь стандартное сообщение о сохранении не выводится
	 + унифицирована совместимость (кательно неуязвимости при входе) - теперь будет ругаться только при критической ошибке
	 + проверка версий "по стандарту"
 *  
 *  
 * TODO
 * - mobrider
...
public class MonsterFix extends JavaPlugin{
	...
	private static MFUtil util;
	String language = "english";
	boolean lngsave = false;
	boolean vcheck = true;
	...
	Set<String> loadFailed = new HashSet<String>();

	File directory;
	private MFBlockListener bl;
	private MFPlayerListener pl;
	private MFMobFix mfl;
	...
	public void InitCfg(){
		...
		addBool("waterfarm","antifarm",true,"water-farming-fix.enable","Fix farming with water");
		...
		addBool("cocoagrow","antifarm",true,"cocoa-grow-fix.enable","Fix unlimited cocoa growing");
		...
		addBool("lcasecmd","system",true,"lower-case-commands.enable","Use only lower-cased commands");
		addBool("lcasecmdmsg","system",true,"lower-case-commands.show-msg","Inform player that commnad must be in lower case");
		...
		//addBool("createlore","gameplay",true,"creator-lore.enable","Add creator name at the lore of item");
		//Список итемов, кому добавляем lore
		//добавлять ли lore для креативщиков ??
		 ...
		 * Модификации мобов
		 */
		//Спавн вооруженных мобов
		addBool("equip","mob",true,"equipment.armour-enable","Spawn more mobs equiped with armour");
		addBool("eqegg","mob",true,"equipment.spawner-egg","Allow to equip mobs spawned from spawner egg");
		addBool("eqmobspawn","mob",true,"equipment.mobspawner","Allow to equip mobs spawned from mob spawner");
		addBool("eqcustom","mob",true,"equipment.custom-plugin","Allow to equip mobs spawned from custom source (other plugins)");
		addBool("eqfull","mob",true,"equipment.armour-full-set","Spawn mobs with full set of armour");
		addBool("eqarmcolor","mob",true,"equipment.armour-colored-leather","Colorize leather armour (Funny zombies!)");
		addStr("eqarmmob","mob","SKELETON,ZOMBIE","equipment.armour-mobs","Mobs that will be spawned with armour");
		addStr("eqwpnmob","mob","ZOMBIE","equipment.weapon-mobs","Mobs that will be spawned with weapon");
		addBool("eqweapon","mob",true,"equipment.weapon-enable","Spawn more mobs equiped with weapon");
		addInt("eqchance","mob",30,"equipment.chance","Chance to spawn mob with equipment");
		addInt("eqdropchance","mob",20,"equipment.drop-chance","Chance of mob equipment drop");


		addStr("eqhelm","mob","LEATHER_HELMET,CHAINMAIL_HELMET,GOLD_HELMET,IRON_HELMET,DIAMOND_HELMET,PUMPKIN","equipment.helmet","Helmet item list");
		addStr("eqchest","mob","LEATHER_CHESTPLATE,CHAINMAIL_CHESTPLATE,GOLD_CHESTPLATE,IRON_CHESTPLATE,DIAMOND_CHESTPLATE","equipment.chestplate","Chestplate item list");
		addStr("eqleg","mob","LEATHER_LEGGINGS,CHAINMAIL_LEGGINGS,GOLD_LEGGINGS,IRON_LEGGINGS,DIAMOND_LEGGINGS","equipment.leggings","Leggings item list");
		addStr("eqboot","mob","LEATHER_BOOTS,CHAINMAIL_BOOTS,GOLD_BOOTS,IRON_BOOTS,DIAMOND_BOOTS","equipment.boots","Boots item list");

		addStr("eqweapon","mob","267,268,272,276,283,271,275,279,286,258","equipment.weapon","Weapon item list");

		// Зачарования
		addBool("eqench","mob",true,"equipment.enchant.enable","Enchant mob's armour and weapon");
		addInt("eqenchance","mob",30,"equipment.enchant.chance","Chance to enchant equipment of spawned mob");
		addInt("eqencmax","mob",4,"equipment.enchant.max-level","Maximum level of enchantment");
		addBool("eqskeleton","mob",true,"equipment.enchant.skeleton-bow","Force enchantment of skeleton's bow");

		addStr("eqenchelm","mob","PROTECTION_ENVIRONMENTAL,PROTECTION_FIRE,PROTECTION_EXPLOSIONS,PROTECTION_PROJECTILE,THORNS,DURABILITY,OXYGEN,WATER_WORKER","equipment.echant.helmette","Possible helmette enchantments");
		addStr("eqencchest","mob","PROTECTION_ENVIRONMENTAL,PROTECTION_FIRE,PROTECTION_EXPLOSIONS,PROTECTION_PROJECTILE,THORNS,DURABILITY","equipment.echant.chestplate","Possible chestplate enchantments");
		addStr("eqencleg","mob","PROTECTION_ENVIRONMENTAL,PROTECTION_FIRE,PROTECTION_EXPLOSIONS,PROTECTION_PROJECTILE,THORNS,DURABILITY","equipment.echant.leggins","Possible leggins enchantments");
		addStr("eqencboot","mob","PROTECTION_ENVIRONMENTAL,PROTECTION_FIRE,PROTECTION_EXPLOSIONS,PROTECTION_PROJECTILE,THORNS,DURABILITY,PROTECTION_FALL","equipment.echant.leggings","Possible leggings enchantments");
		addStr("eqencweapon","mob","DAMAGE_ALL,DAMAGE_UNDEAD,DAMAGE_ARTHROPODS,KNOCKBACK,FIRE_ASPECT,LOOT_BONUS_MOBS","equipment.echant.weapon","Possible weapon enchantments");
		addStr("eqencbow","mob","ARROW_DAMAGE,ARROW_KNOCKBACK,ARROW_FIRE,ARROW_INFINITE,DURABILITY","equipment.echant.bow","Possible bow enchantments (for skeletons only)");
	}
	...
	public void onEnable() {
		...
		pm.registerEvents(this.mfl, this);

		MFObc.init();
		if (MFObc.isBlocked()) {
			log("This version of MonsterFix was not tested with "+Bukkit.getVersion());
			log("Some features will be disabled.");
			log("Check updates at http://dev.bukkit.org/server-mods/monsterfix/");
		}
		...
		try {
			...
			log("Failed to submit stats to the Metrics (mcstats.org)");
		}
		fillPermissions();
	...
	public void reloadCfg(){
		...
		lngsave = getConfig().getBoolean("general.language-save", false);
		getConfig().set("general.language-save", lngsave);
		vcheck = getConfig().getBoolean("general.check-updates", true);
		getConfig().set("general.check-updates", vcheck);
		saveConfig();
	}

	public void loadGroupsCfg() {
		for (String grp : cfggroup.keySet()){
			if (grp.equalsIgnoreCase("config")) continue;
			...
			try {
				...
				u.log("Error while loading configuration from file: ");
				u.log(f.getAbsolutePath());
				e.printStackTrace();
				loadFailed.add(grp);
				continue;
			}
			cfggroup.put(grp, cfg.getBoolean(grp+".enable",true));
			for (MFBool c : cfgb) c.v = cfg.getBoolean(c.grp+"."+c.node, c.v);
			for (MFInt c : cfgi) c.v=cfg.getInt(c.grp+"."+c.node, c.v);
			for (MFStr c : cfgs) c.v=cfg.getString(c.grp+"."+c.node, c.v);
			for (MFFloat c : cfgf) 	c.v=(float) cfg.getDouble(c.grp+"."+c.node, c.v);
		}
	}

	public void saveGroupsCfg(){
		for (String grp : cfggroup.keySet()){
			if (grp.equalsIgnoreCase("config")) continue;
			if (loadFailed.contains(grp)) {
				u.log("Group \""+grp+"\" configuration was not saved. Fix your configuration file and restart the server.");
			}
			YamlConfiguration cfg = new YamlConfiguration();
			File f = new File (getDataFolder()+File.separator+grp+".yml");
			boolean saveComments = f.exists();
			if (saveComments)
				try {
					cfg.load(f); // Сохраняем предыдущие ключи и комментарии.
				} catch (Exception ignore) {
				}
			cfg.set(grp+".enable", cfggroup.get(grp));
			saveGroupCfg (cfg,grp);
			try {
				cfg.save(f);
			} catch (IOException e) {
				u.log("Error while saving configuration to file: "+f.getAbsolutePath());
				e.printStackTrace();
			}
			if (!saveComments) reCommentFile (grp);
		}
	}
	...
	public void saveGroupCfg (YamlConfiguration cfg, String grp){
		for (MFBool c : cfgb) {
			if (!c.grp.equals(grp)) continue;
			//cfg.set(c.grp+"."+c.node+"_description",u.getMSGnc(c.txt));
			cfg.set(c.grp+"."+c.node+"_description",c.txt);
			cfg.set(c.grp+"."+c.node,c.v);
		}
		for (MFInt c : cfgi) {
			if (!c.grp.equals(grp)) continue;
			cfg.set(c.grp+"."+c.node+"_description",c.txt);
	...
	public void SaveTick(){
		...
		if (cfgB("savemsg")) u.broadcastMSG("config", "msg_savingall",'f');
		if (getServer().getOnlinePlayers().size()>0) getServer().savePlayers();
	...
	protected boolean cfgB (String name){
		...
		log("[mfix] Variable "+name+" was not found in cfgb-list!");			
		return false;
	}

	protected boolean cfgBAny(String... names){
		for (String name : names) for (MFBool bc : cfgb){ 
			if ((bc.name.equalsIgnoreCase(name))&&
					(cfggroup.containsKey(bc.grp))&&
					cfggroup.get(bc.grp)&&
					bc.v) return true;
		}
		return false; 
	...
	protected int cfgI (String name){
		...
		log("[mfix] Variable "+name+" was not found in cfgi-list!");
		return -1;
	}
	...
	protected String cfgS (String name){
		...
		log("[mfix] Variable "+name+" was not found in cfgs-list!");
		return "";
	}
	...
	protected float cfgF (String name){
		...
		log("[mfix] Variable "+name+" was not found in cfgf-list!");				
		return 0f;
	}
		...
		if (!bl.isEmpty()){
			...
			if (bln.length>0)
				for (int i = 0; i< bln.length;i++){
					...
					else log("[mfix] Wrong biome name: "+bln[i]);
				}
		}
	}
	...
	public void saveHlpFile(){
		try {
			...
			for (MFBool c : cfgb) hlp.set(c.grp+"."+u.getMSGnc(c.txt)+".command","/mfix "+c.name+"=<on|off> (default: "+c.v+")");
			for (MFInt c : cfgi) hlp.set(c.grp+"."+u.getMSGnc(c.txt)+".command","/mfix "+c.name+"=<integer value> (default: "+c.v+")");
			for (MFFloat c : cfgf) hlp.set(c.grp+"."+u.getMSGnc(c.txt)+".command","/mfix "+c.name+"=<float value> (default: "+c.v+")");
			for (MFStr c : cfgs) hlp.set(c.grp+"."+u.getMSGnc(c.txt)+".command","/mfix "+c.name+"=<string> (default: "+c.v+")");
			hlp.save(f);
		} catch (Exception e){
			e.printStackTrace();
		}

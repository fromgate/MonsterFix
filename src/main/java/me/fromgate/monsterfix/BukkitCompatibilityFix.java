...
public class BukkitCompatibilityFix {
	...
	public static LivingEntity getShooter (Projectile projectile){
		...
		try {
			...
			MonsterFix.getUtil().logOnce("getShooter", "Looks like this version of BukkitAPI totally incompatible with API 1.7.x. Method \"getShooter\" is not declared in Projectile class");
		}
		return null;
	}
	...
	private static double getDoubleMethod (String methodName, Object object, Class<?> clazz){
		...
		try {
			...
			MonsterFix.getUtil().logOnce("BCF"+methodName, "Looks like this version of BukkitAPI totally incompatible with API 1.7.x. Method \""+methodName+"\" is not declared in "+object.getClass().getCanonicalName());
			e.printStackTrace();
		}
		return 0;
	}
	...
	private static void executeMethodObjectDouble (String methodName, Object object, Class<?> clazz, double param){
		...
		} else MonsterFix.getUtil().logOnce("BCFix"+methodName, "Looks like this version of BukkitAPI totally incompatible with API 1.7.x. Method \""+methodName+"\" is not declared in "+object.getClass().getCanonicalName()+" or not working properly");
	}
	...
	public static EntityEvent createEntityDamageByEntityEvent (Entity damager, Entity entity, DamageCause damageCause, double damage){
		...
		try {
			...
			try {
				...
				MonsterFix.getUtil().logOnce("BCFixEntityDamageEvent", "Failed to determine constructor for EntityDamageByEntityEvent");
				return null;
			}
		}
		Object event = null;
		try {
			...
			MonsterFix.getUtil().logOnce("BCFixEntityDamageEvent2", "Failed to initiate EntityDamageByEntityEvent");
			return null;
		}
